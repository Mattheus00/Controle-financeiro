...
import { getMerchantBrandCatalog } from "@/features/merchants/services/merchantBrandService";
import { MerchantBrandProvider } from "@/features/merchants/merchant-brand-provider";

export default async function AppLayout({ children }: { children: React.ReactNode }) {
  const { supabase, userId } = await requireUser();
  const [{ data: profile }, { data: userData }, catalog] = await Promise.all([
    supabase.from("profiles").select("name").eq("user_id", userId).maybeSingle(),
    supabase.auth.getUser(),
    getMerchantBrandCatalog(supabase),
  ]);

  await recurringService.generateUpcoming(supabase, userId);

  return (
    <MerchantBrandProvider brands={catalog.brands}>
